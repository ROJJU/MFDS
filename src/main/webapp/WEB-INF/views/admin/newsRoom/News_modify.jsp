	...
	<tr>
		<td height="200" align="center" bgcolor="#FFFFFF" class="menu" id="sub_content">
			<form id="boardWriteForm" method="post" enctype="multipart/form-data">
			    <div style="text-align:left;">
				   ...
				   <input type="hidden" value="${news.n_file}" name="n_file_old">
				       &nbsp; * 현재 첨부 파일 :
				       <c:choose>
				       		<c:when test="${news.n_file==null}">첨부된 파일이 없습니다.</c:when>
				       		<c:otherwise>${news.n_file} (첨부파일을 선택하지 않을 경우 현재 첨부된 파일이 그대로 업로드 됩니다.)</c:otherwise>
				       </c:choose>
				</div>
			...
			<div class="buttonDiv">
			    ...
			    <input type="image" src="/resources/img/btn/cancel_button.png" onclick="goUrl('/AdminNews.do?page_seq=1&side_seq=0');" width="60px">
		 	</div><br><br><br><br>
		</td>
    </tr>
</table>
...
<script type="text/javascript">
	...
	var onWrite = function(){
	 ...
	 boardWriteForm.action ="/NewsModifyProc.do";  //저장할 페이지로 쏩니다. db를 쓰지 않으니 사용하지 않았습니다.
