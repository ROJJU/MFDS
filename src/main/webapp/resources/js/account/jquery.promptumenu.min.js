...
(function(m){
	m.fn.promptumenu=function(d){
		var f=m.extend({columns:3,rows:4,direction:"horizontal",width:"auto",height:"auto",duration:500,pages:!0,inertia:200},d);
			return this.each(
					function(){
						var d,k,u,v,b=m(this),e,c={x:0,y:1,page:1},g=0,j=0,h=1,q=1,n={go_to:function(a,r,c){void 0===r&&(r="swing");
						void 0===c&&(c=!1);
						var h,d;"vertical"==f.direction?(h={top:-1*(a-1)*e.height
					},
					d={"-webkit-transform":"translate3d(0px, "+-1*(a-1)*e.height+"px, 0px)"}):(h={left:-1*(a-1)*e.width},
					d={"-webkit-transform":"translate3d("+-1*(a-1)*e.width+"px, 0px, 0px)"});
					c?(b.css({"-webkit-transition-property":"-webkit-transform","-webkit-transition-duration":f.duration+"ms","-webkit-transition-timing-function":"ease-out"}),
					b.css(d),b.data("ppos",-1*(a-1)*e.width)):b.animate(h,f.duration,r);b.parent(".promptumenu_window").find(".promptumenu_nav a.active").removeClass("active");
					b.parent(".promptumenu_window").find(".promptumenu_nav a:nth-child("+a+")").addClass("active");q=a},next_page:function(){n.go_to(q+1)},
					prev_page:function(){n.go_to(q-1)}};
						if(b.data("promptumenu"))console.error("You are calling promptumenu for an element more than twice. Please have a look.");
						else{b.data("promptumenu",!0);
					b.data("ppos",0);e={width:"auto"==f.width?b.width():f.width,height:"auto"==f.height?b.height():f.height,padding:0,display:"block",overflow:"hidden"};
					g=e.width/f.columns;
					j=e.height/f.rows;
					b.wrap('<div class="promptumenu_window" />');
					b.parent(".promptumenu_window").css({width:"1000px", padding:"0px", display:"block", overflow:"hidden"});
					b.css({display:"block",position:"relative","list-style":"none",overflow:"visible",height:"auto",width:"auto",top:0,left:0,margin:0,padding:0});
					b.children("li").css({display:"block",position:"absolute",margin:0});
					b.children("li").each(function(){var a=m(this);
					c.x+=1;if(c.x>f.columns)c.x=1,c.y+=1;if(c.y>f.rows)c.x=1,c.y=1,c.page+=1;a.data("layout",m.extend({},c));
					"vertical"==f.direction?(a.css({top:Math.round(c.y*j-j/2-a.height()/2+(c.page-1)*e.height),left:Math.round(c.x*g-g/2-a.width()/2)}),a.find("img").bind("load",function(){var b=a.data("layout");a.css({top:Math.round(b.y*j-
